for _ in range(0, int(input())):
    ...
    sa += min( len((a-b)-c) , len((a-c)-b) )*3
    sb += min( len((b-a)-c) , len((b-c)-a) )*3
    sc += min( len((c-a)-b) , len((c-b)-a) )*3
    # print(sa, sb, sc)
    sa += ( len((a&b)-c) + len((a&c)-b))
    sb += ( len((b&a)-c) + len((b&c)-a))
    sc += ( len((c&a)-b) + len((c&b)-a))
