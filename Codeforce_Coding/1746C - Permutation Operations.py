...
A permutation of size n is an array of size n such that each integer from 1 to n occurs exactly once in this array. A suffix is several consecutive elements of an array that include the last element of the array. An inversion in an array a is a pair of indices (i,j) such that i>j and ai<aj.
...
The second line contains n distinct integers a1,a2,…,an (1≤ai≤n), the initial permutation a.

It's guaranteed that the sum of n over all test cases does not exceed 2⋅105.

Output
For each test case, print n integers x1,x2,…,xn (1≤xi≤n for each 1≤i≤n) indicating that the i-th operation must be applied to the suffix starting at index xi. If there are multiple answers, print any of them.
